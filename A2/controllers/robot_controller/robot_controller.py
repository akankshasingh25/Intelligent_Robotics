...
while robot.step(TIME_STEP) != -1 and state != 'reached_goal':
    ...
    if right_value < 0.3 and right_value < left_value and right_value < front_value:
        left_speed -= 0.25 * MAX_SPEED * (0.3 - right_value) / 0.3
        right_speed += 0.25 * MAX_SPEED * (0.3 - right_value) / 0.3
        print("Right Obstacle Detected")
        
    # closest obstacle to the left
    if left_value < 0.3 and left_value < right_value and left_value < front_value:
        left_speed += 0.25 * MAX_SPEED * (0.3 - left_value) / 0.3
        right_speed -= 0.25 * MAX_SPEED * (0.3 - right_value) / 0.3
